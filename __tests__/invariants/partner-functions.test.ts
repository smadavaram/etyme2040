...
const NIKE = { id: 'c-nike', name: 'Northbend Athletic Inc' }
const NIKE_SSC = { id: 'c-ssc', name: 'Northbend Athletic Shared Services BV' }
const NIKE_TREASURY = { id: 'c-tre', name: 'Northbend Athletic Treasury Ltd' }
const BEAVERTON = {
...
describe('Four parties, one client, and only one of them needs setting up', () => {
  it('when nobody has said otherwise, all four partner functions are the client on the agreement', () => {
    ...
    expect(p.says).toContain('Northbend Athletic Inc throughout')
  })
  ...
  it('an invoice is addressed to the bill-to and settled by the payer, and they are allowed to differ', () => {
    ...
    // The agreement is still with Northbend Athletic Inc — that is what was signed.
    expect(p.soldTo.party).toEqual(NIKE)
    expect(p.split).toBe(true)
  })

  it('a contract beats the engagement, because the most specific thing somebody said wins', () => {
    const other = { id: 'c-oth', name: 'Northbend Athletic Retail GmbH' }
    const p = partnerFunctions({
      agreementClient: NIKE,
...
describe('Consolidated billing joins contracts, never companies', () => {
  it('several contracts to one bill-to in one currency go on one invoice', () => {
    const v = mayConsolidate([
      { sellContractId: 's1', billToId: 'b', billToName: 'Northbend Athletic SSC', payerId: 'p', currency: 'USD' },
      { sellContractId: 's2', billToId: 'b', billToName: 'Northbend Athletic SSC', payerId: 'p', currency: 'USD' },
      { sellContractId: 's3', billToId: 'b', billToName: 'Northbend Athletic SSC', payerId: 'p', currency: 'USD' },
    ])
  ...
  it('one invoice covering several contracts still names one bill-to, and refuses to consolidate across two of them', () => {
    const v = mayConsolidate([
      { sellContractId: 's1', billToId: 'b1', billToName: 'Northbend Athletic SSC', payerId: 'p', currency: 'USD' },
      { sellContractId: 's2', billToId: 'b2', billToName: 'Northbend Athletic Retail', payerId: 'p', currency: 'USD' },
    ])
  ...
  it('contracts billing in two currencies are never consolidated, because that total is a total of nothing', () => {
    const v = mayConsolidate([
      { sellContractId: 's1', billToId: 'b', billToName: 'Northbend Athletic SSC', payerId: 'p', currency: 'USD' },
      { sellContractId: 's2', billToId: 'b', billToName: 'Northbend Athletic SSC', payerId: 'p', currency: 'EUR' },
    ])
    expect(v.ok).toBe(false)
  ...
  it('contracts settled by two different payers are refused', () => {
    const v = mayConsolidate([
      { sellContractId: 's1', billToId: 'b', billToName: 'Northbend Athletic SSC', payerId: 'p1', currency: 'USD' },
      { sellContractId: 's2', billToId: 'b', billToName: 'Northbend Athletic SSC', payerId: 'p2', currency: 'USD' },
    ])
    expect(v.ok).toBe(false)
