    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.automationLog.create({
        data: {
          ...
          summary: `Invoice ${number} generated: ${lines.length} line item(s), ${allTimesheetIds.length} timesheet(s)${expLines.length ? `, ${expLines.length} expense(s)` : ''}${milestones.length ? `, ${milestones.length} milestone(s)` : ''}, ${fromUnits(total, currency)} total, due ${dueAt.toISOString().slice(0, 10)}${pendingSays ? `. ${pendingSays}` : ''}`,
    ...
    return NextResponse.json({
      data: {
        ...
        tax: tax
          ...
          : null,
        taxNote,
        message: `Invoice ${result.number} generated: ${fromUnits(Number(result.total), result.currency)} total`,
      },
    }, { status: 201 })
  } catch (err: any) {
    if (err?.code === 'P2002') {
