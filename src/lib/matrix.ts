      ...
      { code: 'L2.1.4', name: 'Reaching the market, and moving work between firms', domain: 'MARKET', processes: [
        ...
        { code: 'L3.1.4.6', name: 'The contractor census', owner: 'Etyme', status: P,
          implementedBy: [
            ...
            'src/app/api/census/request/route.ts',
            'src/app/api/census/agree/route.ts',
            'src/app/api/census/upload/route.ts',
            'src/app/api/census/review/route.ts',
            'src/app/api/census/import/route.ts',
            'src/app/api/census/page/route.ts',
            'src/app/legal/census-agreement/page.tsx',
            'src/lib/census-copy.ts',
            'src/app/census/page.tsx',
            'src/app/census/flow.tsx',
            'src/lib/legal.ts',
            'src/lib/retention.ts',
            'src/lib/data-request.ts',
            'src/lib/erasure.ts',
            'src/lib/notify/census.ts',
            'src/lib/autonomy.ts',
            'public/census-template.csv',
          ],
          testedBy: [
            '__tests__/invariants/census.test.ts',
            '__tests__/invariants/census-import.test.ts',
            '__tests__/invariants/census-page.test.ts',
            '__tests__/invariants/census-page-copy.test.ts',
            '__integration__/census.test.ts',
            '__integration__/census-page.test.ts',
            '__tests__/invariants/census-letters.test.ts',
            '__tests__/invariants/autonomy.test.ts',
            '__tests__/invariants/legal-pages.test.ts',
          ],
          tasks: [
            'Platform — a staff member with no seat can open the review, and gains no permission inside any company',
            'Market DONE \u2014 /census says what you get and what you send, with no price, no AI claim and no invented urgency \u2014 src/lib/census-copy.ts, src/app/census/page.tsx, src/app/census/flow.tsx',
            'Market DONE \u2014 the template downloads as CSV with a header row and one example row, because there is no spreadsheet library and none is added for this \u2014 public/census-template.csv, reached from src/app/census/flow.tsx',
            'Regulatory DONE — the census agreement is the fourth legal document, rendered through the same renderer as the terms, the privacy notice and the DPA, with the six headings a procurement lead asks for: what we receive, who at Etyme can see it, where it sits, that we never approach their suppliers or contractors, the day we delete it, and what happens if they start a program \u2014 src/lib/legal.ts, src/app/legal/census-agreement/page.tsx',
            'Regulatory DONE \u2014 a client asks with no account and nothing about them is verified: no Person, no Company, and a consumer address is refused in a sentence that says a census is the company\u2019s own data about its own contractors \u2014 src/app/api/census/request/route.ts',
            'Regulatory DONE \u2014 nothing moves until somebody at the client accepts by name: the upload token does not exist until acceptance, so there is nowhere to send a file to, and the edition they accepted is written to agreementVersion because which edition somebody signed is the finding in an audit',
            'Regulatory DONE \u2014 the upload link expires on the row rather than inside a signature, copying the DocumentPacket pattern, so legal changing its mind revokes it; accepting twice hands back the link that exists rather than minting a second one nobody can revoke',
            'The template asks for no names, and a census from the template holds no personal data beyond a work email',
            'Regulatory DONE \u2014 two gates, not one: being Etyme staff opens the queue, and opening the bytes of a client\u2019s file needs assignedStaffEmail to match, because what the client was told is a name and \u201cour staff\u201d is not a name. Every open and every refusal writes an AccessLog row per contractor, awaited before the bytes go out \u2014 src/app/api/census/review/route.ts',
            'Money DONE — the template is a header row and one example row and asks for no names: supplier, role, site, the two dates, the rate, hours a week and the client\'s own reference number; public/census-template.csv and the TEMPLATE_CSV the importer reads are held together by a test, so what a client downloads is what this code parses',
            'Money DONE — one row per contractor imported into a sandbox company of the client\'s own as contracts and placements, every gap recorded against the line it was on and never defaulted away — src/lib/census-import.ts, src/app/api/census/import/route.ts',
            'Money DONE — the one page is the brief\'s six sections in its order: who is on site and through whom, the quarter\'s spend and each supplier\'s share, where two suppliers charge differently for one skill, who has been here longest, what we could not see, and how this was computed — print-styled on one sheet, src/lib/census-page.ts and src/app/api/census/page/route.ts',
            'Money DONE — the supplier rate spread is the only new arithmetic on the page, because rate variance in this product is by hiring manager and cannot see that two firms are paid differently for one skill: lowest, highest and the gap, and a role one supplier fills says so instead of showing a gap of zero',
            'Money DONE — the other three numbers are imported and never reimplemented: the head count is chainTop so a person bought through a chain is one contractor and not one per rung, the days on site are lib/tenure-days as the union of the periods, and supplier names go through lib/chain-names so this page never gives a prime\'s sub-vendor away',
            'Money DONE — the head count and the supplier head counts on the page are the numbers the client dashboard shows in the same sandbox, proved on twelve rows across three suppliers in __integration__/census-page.test.ts',
            'Money DONE — one placement with no rate blanks the quarter for the whole book rather than averaging it in at nothing, and the blank names how many rows and why; a book priced in two currencies has no total, because nothing here converts one into another',
            'Money DONE — the page never hides a gap: a row with no end date appears in "what we could not see", a role a third supplier never priced says the real gap can only be wider, and a clean file gets "Every row was complete" out loud',
            'Money OPEN — the quarter\'s spend uses the hours a week the client sent, not the dashboard\'s 160-hour month, and says so; making the two one function needs the 160-hour month inlined at src/app/api/program/route.ts extracted into lib/ by Demand, who owns that file — asked for, not worked around',
            'Money OPEN — a placement whose rate the client never sent is stored as a zero because SellContract.billRate is not nullable, so the sandbox dashboard prices that person at nothing while the page blanks the book; a nullable column is the fix',
            'Regulatory DONE \u2014 one date in one column: deleteBy is set once at receipt, forty-five days out (five working days of review plus the thirty days with the page), and the confirmation, the page and the nightly sweep all read it because there is nowhere else to read it from. A second upload does not move it and mayResetDeleteBy says why in words',
            'Regulatory DONE \u2014 past its day with no program started, the nightly sweep deletes the files, the rows imported from them, the supplier companies and the sandbox itself, sets deletedAt and DELETED, and keeps the file count and byte total so the day after we can still say what went \u2014 runCensusSweep in src/lib/data-request.ts; CENSUS_CLOCK_WARNED tells the named person once a night while the page is unsent',
            'Regulatory DONE \u2014 a program starting is the only thing that changes the date, and it is a cancellation recorded with a reason, not a move: CENSUS_DELETION_CANCELLED is on the autonomy ladder at L3 and the sweep leaves the census alone however far past its day it is',
            'Regulatory DONE \u2014 all seven census automation names are on the autonomy ladder and written for real. AutomationLog.companyId went nullable on 2026-09-20 and every census row carries no company at all \u2014 an act of the platform, outside any tenant \u2014 which is what lets the deletion row outlive the sandbox the deletion destroys. CENSUS_DELETED is L5 and reversible false; CENSUS_CLOCK_WARNED is L0; the four a person took are ATTRIBUTED \u2014 src/lib/autonomy.ts',
            'Regulatory DONE \u2014 a file opened before any import leaves a record that outlives the file: CensusRead writes one row per open and per refusal, keyed on the census rather than on a person, with the file\u2019s name as a plain column and no foreign key so the line still says \u201cQ3 Veritan invoices.pdf\u201d a year after the file is deleted. The per-person AccessLog rows stay as they were after the import \u2014 src/app/api/census/review/route.ts',
            'Regulatory DONE \u2014 the clock warning is remembered on CensusRequest.lastWarnedAt and nowhere else, so it goes once a night rather than once a run even where the named person holds no Person row. Counting notifications was wrong the day a second letter shared the same entityId: an arrival silenced the clock for that day, quietly, on the census closest to its date',
            'Regulatory OPEN \u2014 cron/retention\u2019s morning sentence does not mention the census counts; runRetentionSweep returns censusDeleted and censusWarned and the route builds its says from the older fields. Staff are told by email either way. One line in a Platform file',
            'Conversation DONE \u2014 the seven letters are written as pure functions and every date in them is read off the row, never recomputed \u2014 src/lib/notify/census.ts, __tests__/invariants/census-letters.test.ts',
            'Regulatory DONE \u2014 the five client letters are wired at the five moments: asked and the staff arrival in POST /api/census/request, agreed in POST /api/census/agree on both the first acceptance and the second, received in POST /api/census/upload, delivered in POST /api/census/review, deleted in runCensusSweep. A census contact has no Person, so every one goes through emailSender() with the address on the row',
            'Regulatory DONE \u2014 with no email sender configured the letter becomes a staff instruction \u2014 \u201cSend to dana@\u2026: Your census files arrived\u201d with the letter under it \u2014 rather than a silent drop, which is the sendOutside pattern the erasure letter already uses for an address nothing can reach',
            'Regulatory DONE \u2014 a client who lost the tab between accepting and uploading gets the upload link again by pressing accept, and it is the same link, because two live links to one census is one more than anybody can revoke',
            'Regulatory DONE \u2014 the client reads a client\u2019s sentence about who runs their census: assignmentSays names ETYME_STAFF_EMAILS and is staff-only, assignedSaysToClient says a person picks it up and writes to you by name',
            'No email invents a deadline, and there is no nurture sequence — the shipped promise is that nothing automatic happens next. The census agreement in lib/legal now names the five moments out loud rather than saying nothing automatic happens, because five letters that answer something the client did are not nothing',
      ...
      { code: 'L2.1.3', name: 'Evaluation', domain: 'DEMAND', processes: [
        ...
        { code: 'L3.1.3.3', name: 'Award and seat close', owner: 'Program manager', status: B,
          tasks: ['Governance checks', 'Seat arithmetic', 'Raise both sides of the deal', 'Stand down the rest',
            'Raise the purchase order with the contract as its first line, in one transaction — one open order per buyer and seller, found rather than raised twice',
            'The ceiling is what the requisition was approved at, carrying the basis it came from; where nothing supports a figure no order is raised and the answer says why',
            'A sub-vendor’s line hangs on the prime’s own order to the sub, and an employee’s line hangs on no order at all'],
          implementedBy: ['src/lib/award.ts', 'src/app/api/submissions/[id]/award/route.ts',
            'src/app/api/submissions/order-header.ts'],
          testedBy: ['__tests__/invariants/award.test.ts', '__tests__/invariants/award-order.test.ts',
  ...
  },
  {
    ...
    groups: [
      { code: 'L2.2.1', name: 'Commercial papering', domain: 'DEMAND', processes: [
        { code: 'L3.2.1.1', name: 'Agreement and SOW — the legal umbrella', owner: 'Account manager', status: P,
          tasks: ['Terms, margin floor, payment days — warns with a reason code, never blocks',
            ...
            'An agreement is the umbrella and nothing more — a client that sends one order and one contractor is never made to paper one first: `Engagement.msaId` is optional, no route invents an agreement, and an invoice says who it is between through the order and the lines billed on it',
            'NOT YET: nothing records an agreement two firms actually negotiated. `api/clients`, `api/suppliers` and `api/supplier-requests/[id]` write an unsigned stub with thirty-day terms so a counterparty has a row, and `api/program/agreements` only reads, signs, amends and ends what is already there — so a term, payment days, a margin floor and the sub-vendor disclosure clause can be agreed at signing and typed in nowhere',
            'A screen says “agreement” or “MSA” and never prefixes it with “master” — that word belongs to the profitability roll-up, and two things that sound like one are two things nobody can tell apart'],
          implementedBy: [
            ...
            'src/lib/money/invoice-parties.ts',
            'src/app/api/submissions/[id]/award/route.ts',
          ...
          testedBy: ['__tests__/invariants/agreements.test.ts', '__tests__/invariants/schema-terms.test.ts',
            ...
            '__integration__/agreement-lifecycle.test.ts', '__integration__/award-order.test.ts'] },
        { code: 'L3.2.1.2', name: 'Sell contract — the line the firm bills from', owner: 'Account manager', status: B,
          tasks: ['Bill rate, term, end client', 'Cost object and PO attached at award',
            'It is a line on an order, never a document of its own — one order for five people is one document and five lines',
            'Somebody taking over a seat is a line on the same order the person who left it was on, billed on the document’s own rhythm rather than on the old line’s copy of it',
            'A line not on an order yet says so in a sentence and says what will attach, rather than showing a blank',
            'The award writes the line and its order together, and the older convert path writes the same pair onto the same document rather than a second one',
            'A line never disagrees with its header on the day it is written: the four rhythm and terms fields come down from the order, and the dates stay the line’s and sit inside the order’s window'],
          implementedBy: ['src/app/api/submissions/[id]/award/route.ts', 'src/app/api/contracts/route.ts',
            'src/lib/replacement.ts', 'src/app/api/placements/[id]/replace/route.ts', 'src/lib/order-naming.ts',
            'src/app/api/submissions/order-header.ts', 'src/app/api/submissions/[id]/convert/route.ts'],
          testedBy: ['__tests__/invariants/contracts.test.ts', '__tests__/invariants/award.test.ts',
            '__tests__/invariants/order-lines.test.ts', '__tests__/invariants/replacement.test.ts',
            '__tests__/invariants/award-order.test.ts', '__integration__/award-order.test.ts'] },
        { code: 'L3.2.1.3', name: 'Buy contract or employment agreement', owner: 'Delivery manager', status: B,
          tasks: ['Pay model, share basis, burden', 'Raised at award, never after',
            ...
            'It is the line the firm pays from — a sub-vendor’s invoice, received and matched, or payroll where the firm employs the person, and a screen says which',
            'No order is raised to an employee, so a W2 line has no header and is complete without one'],
          ...
          testedBy: ['__tests__/invariants/pay-model.test.ts', '__tests__/invariants/award.test.ts', '__tests__/invariants/schema-terms.test.ts',
            '__tests__/invariants/payroll-export.test.ts', '__tests__/invariants/order-lines.test.ts'] },
        { code: 'L3.2.1.4', name: 'The work order — one row, named per viewer', owner: 'Procurement', status: B,
          tasks: [
            ...
            'A line on it is named by the person and the site — “Purchase order PO-4471, line 2 of 5” to the client, “Sales order SO-0913, line 2” to the supplier — and never by a contract id',
            'A placement shows the document it hangs on: the ceiling, what has been billed against it, and everybody else on the same paper',
            'The award raises it with its first line, so nobody has to remember to — and a second award between the same two firms joins it rather than opening another',
            'A line reads its billing rhythm and its payment days from the document it is on, and keeps its own dates, because an order is not a person',
          ],
          implementedBy: [
            ...
            'src/app/api/placements/[id]/route.ts', 'src/app/dashboard/placements/[id]/page.tsx',
            'src/lib/award.ts', 'src/app/api/submissions/order-header.ts',
            'src/app/api/submissions/[id]/award/route.ts',
            'src/lib/money/order-terms.ts', 'src/lib/billing-cascade.ts', 'src/lib/contract-cycles.ts',
            'src/lib/invoice-match.ts', 'src/app/api/invoices/generate/route.ts', 'src/app/api/payroll/route.ts',
          ],
          testedBy: [
            '__tests__/invariants/purchase-order.test.ts', '__tests__/invariants/work-order.test.ts',
            '__tests__/invariants/order-lines.test.ts', '__tests__/invariants/award-order.test.ts',
            '__tests__/invariants/order-terms.test.ts', '__tests__/invariants/terms-through-one-door.test.ts',
  ...
  },
  {
    ...
    groups: [
      { code: 'L2.4.1', name: 'Billing', domain: 'MONEY', processes: [
        ...
        { code: 'L3.4.1.2', name: 'Who is who on an invoice', owner: 'AR clerk', status: B,
          tasks: ['Sold-to, bill-to, ship-to, payer', 'Consolidated billing refuses to mix bill-tos, payers or currencies', 'Self-billing flagged',
            'The two firms come off the agreement, else the order, else the lines billed on it — an agreement is optional and a bill with none still knows who it is between',
            'An invoice nothing can attribute names nobody, lands in no total, and refuses a payment rather than recording one that cannot say who paid whom',
            'The list names the firm on the OTHER end: a client reads the supplier that billed it, a supplier reads the client that owes it, and nobody reads their own name under a column headed Client',
            'A client opens the list on what it owes, because a client never sells \u2014 and the chip that says Showing moves the rows as well as the totals'],
          implementedBy: ['src/lib/billing-cascade.ts', 'src/lib/money/invoice-parties.ts',
            'src/app/api/invoices/generate/route.ts', 'src/app/api/invoices/route.ts',
            'src/app/api/invoices/[id]/route.ts', 'src/app/api/invoices/[id]/payments/route.ts',
            'src/app/api/invoices/[id]/coding/route.ts', 'src/app/api/invoices/[id]/received/route.ts',
            'src/app/dashboard/invoices/page.tsx'],
          testedBy: ['__tests__/invariants/partner-functions.test.ts', '__tests__/invariants/billing-cascade.test.ts',
            '__tests__/invariants/invoice-parties.test.ts'] },
      ...
      { code: 'L2.4.2', name: 'Accounts receivable', domain: 'MONEY', processes: [
        { code: 'L3.4.2.1', name: 'AR aging and dunning', owner: 'AR clerk', status: B,
          tasks: ['Buckets from the due date, not the invoice date', 'Four letters and then a person', 'One letter per customer, not per invoice',
            'The clerk who bills the client can open the page they bill from \u2014 the gate is the desk\u2019s own permission, never margin',
            'The book is what this firm billed, said by the agreement, the order or a line on it \u2014 a bill with no agreement behind it is still chased',
            'Two customers nobody can name are never added together: an unattributed invoice gets a bucket of its own'],
          implementedBy: ['src/lib/money/desks.ts',
            'src/lib/ar-ageing.ts', 'src/lib/money/invoice-parties.ts', 'src/app/api/ar/route.ts',
            'src/app/api/ar/book.ts', 'src/app/api/ar/dunning/route.ts',
            'src/app/dashboard/ar/page.tsx'],
          testedBy: ['__tests__/invariants/money-desks.test.ts',
            '__tests__/invariants/ar-ageing.test.ts',
            '__tests__/invariants/invoice-parties.test.ts'] },
        { code: 'L3.4.2.2', name: 'Cash application', owner: 'AR clerk', status: B,
          ...
          implementedBy: ['src/lib/money/desks.ts',
            'src/lib/ar-ageing.ts', 'src/app/api/ar/payments/route.ts',
            'src/app/api/ar/route.ts', 'src/app/api/ar/book.ts', 'src/app/dashboard/ar/page.tsx'],
          testedBy: ['__tests__/invariants/money-desks.test.ts',
            '__tests__/invariants/cash-application.test.ts', '__tests__/invariants/ar-ageing.test.ts'] },
        { code: 'L3.4.2.3', name: 'Disputes and credit notes', owner: 'Account manager', status: B,
          ...
          implementedBy: ['src/lib/money/desks.ts',
            'src/lib/ar-ageing.ts', 'src/lib/gl.ts', 'src/app/api/ar/credit-notes/route.ts',
            'src/app/api/ar/book.ts', 'src/app/dashboard/ar/page.tsx'],
          testedBy: ['__tests__/invariants/money-desks.test.ts',
            '__tests__/invariants/credit-notes.test.ts', '__tests__/invariants/gl.test.ts'] },
      ]},
      { code: 'L2.4.3', name: 'Credit management', domain: 'MONEY', processes: [
        { code: 'L3.4.3.1', name: 'Exposure, not a credit score', owner: 'Controller', status: B,
          ...
          implementedBy: ['src/lib/money/desks.ts',
            'src/lib/credit.ts', 'src/app/api/ar/route.ts', 'src/app/dashboard/ar/page.tsx'],
          testedBy: ['__tests__/invariants/money-desks.test.ts',
            '__tests__/invariants/credit.test.ts'] },
        { code: 'L3.4.3.2', name: 'Limit and breach', owner: 'Controller', status: B,
          ...
          implementedBy: ['src/lib/money/desks.ts',
            'src/lib/credit.ts', 'src/app/api/ar/credit-limit/route.ts'],
          testedBy: ['__tests__/invariants/money-desks.test.ts',
            '__tests__/invariants/credit.test.ts'] },
        { code: 'L3.4.3.3', name: 'Collections and escalation', owner: 'Controller', status: B,
          ...
          implementedBy: ['src/lib/money/desks.ts',
            'src/lib/ar-ageing.ts', 'src/app/api/ar/collections/route.ts',
            'src/app/dashboard/ar/page.tsx'],
          testedBy: ['__tests__/invariants/money-desks.test.ts',
            '__tests__/invariants/collections.test.ts',
    ...
    groups: [
      ...
      { code: 'L2.5.2', name: 'Accounts payable', domain: 'MONEY', processes: [
        { code: 'L3.5.2.1', name: 'Vendor bill intake', owner: 'AP clerk', status: B,
          ...
          implementedBy: ['src/lib/money/desks.ts',
            'src/lib/purchase-order.ts', 'src/app/api/ap/bills/route.ts',
            'src/lib/cycle-kinds.ts', 'src/app/dashboard/ap/page.tsx',
            'src/app/api/cron/due-cycles/route.ts', 'src/lib/due-cycle-desks.ts'],
          testedBy: ['__tests__/invariants/money-desks.test.ts',
            '__tests__/invariants/vendor-bill-match.test.ts', '__tests__/invariants/purchase-order.test.ts',
            '__tests__/invariants/due-dates-belong-to-the-document.test.ts',
            '__tests__/invariants/due-cycles-desks.test.ts'] },
        { code: 'L3.5.2.2', name: 'Three-way match', owner: 'AP clerk', status: B,
          tasks: ['PO, receipt of work, invoice', 'Tolerance and exception queue', 'Exceptions routed to the AP desk',
            ...
            'A bill for part of a billing period passes \u2014 the receipt is the signed week, and three signed weeks inside one August are a part-period bill, not a period the contract does not have',
            'A bill whose dates cross from one billing period into the next fails, and the sentence names the period it spilled out of so somebody can split it',
            'Every check carries the name a person reads it by, from the engine, so no screen keeps a second list and none of them prints CONTRACT_PERIOD as a heading'],
          implementedBy: ['src/lib/three-way-match.ts', 'src/lib/invoice-match.ts', 'src/app/api/ap/bills/route.ts',
            ...
            'src/app/dashboard/invoices/[id]/page.tsx', 'src/app/api/decisions/route.ts'],
          testedBy: ['__tests__/invariants/three-way-match.test.ts', '__tests__/invariants/vendor-bill-match.test.ts',
            ...
            '__tests__/invariants/partial-period-bill.test.ts', '__tests__/invariants/match-in-words.test.ts'] },
        { code: 'L3.5.2.3', name: 'Payment run and remittance', owner: 'AP clerk', status: B,
          ...
          implementedBy: ['src/lib/money/desks.ts',
            'src/lib/ap-delay.ts', 'src/app/api/ap/payment-runs/route.ts',
            'src/app/dashboard/ap/page.tsx'],
          testedBy: ['__tests__/invariants/money-desks.test.ts',
            '__tests__/invariants/payment-run.test.ts'] },
        { code: 'L3.5.2.4', name: 'Chain payment behavior', owner: 'Controller', status: B,
          tasks: [
            ...
            'What a client paid beside what its supplier was paid is margin, so it is fenced as a figure \u2014 the desk that pays keeps the page it pays from',
            'A client whose suppliers are all on the platform has no vendor bills and still owes money: the page reads the invoices its suppliers raised to it, in one book per currency, and links to where they are paid \u2014 never an empty page saying nothing has been recorded',
          ],
          implementedBy: ['src/lib/money/desks.ts',
            'src/lib/ap-delay.ts', 'src/lib/money/supplier-invoices.ts', 'src/app/api/ap/route.ts',
            'src/app/api/ap/bills/route.ts', 'src/app/dashboard/ap/page.tsx'],
          testedBy: ['__tests__/invariants/money-desks.test.ts',
            '__tests__/invariants/ap-delay.test.ts',
            '__tests__/invariants/a-client-owes-its-suppliers.test.ts'] },
      ]},
  ...
  },
  {
    ...
    groups: [
      { code: 'L2.6.1', name: 'The ledger', domain: 'MONEY', processes: [
        ...
        { code: 'L3.6.1.2', name: 'The master contract — the deal, revenue side and cost side', owner: 'Controller', status: B,
          tasks: ['Collects revenue and cost for one piece of work', 'Allocations per head', 'Basis stated on every line',
            'A person reads it as the master contract — the 2017 word, and the founder’s, for the roll-up a sell line and the buy line that funds it are tagged to',
            'Tagging is the company’s choice and never the system’s: a line on no master contract is a complete line, and the screen offers the tag rather than warning about it',
            'A company opens one by name and moves a line onto it or off it; the company’s tag beats the engagement grouping the system falls back to, and an untagged line still settles somewhere',
            'A settled or closed master contract refuses another line in a sentence, because postings into a reported period are refused',
            'Moving a line never restates what it has already posted, and the reply says so',
            'The budget on a master contract is money and is null for a seat that may not read margin'],
          implementedBy: ['src/lib/order.ts', 'src/lib/order-postings.ts', 'src/lib/order-naming.ts',
            'src/lib/money/master-contract.ts',
            'src/app/api/profitability/master-contracts/route.ts',
            'src/app/api/contracts/[id]/master-contract/route.ts',
            'src/app/dashboard/contracts/page.tsx',
            'src/app/api/placements/[id]/route.ts', 'src/app/dashboard/placements/[id]/page.tsx'],
          testedBy: ['__tests__/invariants/order.test.ts', '__tests__/invariants/order-lines.test.ts',
            '__tests__/invariants/master-contract.test.ts',
            '__tests__/invariants/contracts-as-documents.test.ts'] },
    ...
    groups: [
      ...
      { code: 'L2.7.2', name: 'Commercial risk', domain: 'SUPPLY', processes: [
        { code: 'L3.7.2.1', name: 'Supplier financial risk', owner: 'Procurement', status: B,
          tasks: ['Insurance and payment behavior, where nothing-on-record is never clear',
            ...
            'PARTLY TESTED: the arithmetic is covered and the route is not. `api/vendors/risk` has no integration test of its own — the standing-and-seats tests never call it, and what it shares with `api/vendors/concentration` is the billing cascade an invoice is attributed through, which `__tests__/invariants/concentration.test.ts` now covers'],
          ...
          testedBy: ['__tests__/invariants/supplier-risk.test.ts', '__tests__/invariants/standing-and-seats.test.ts',
            '__tests__/invariants/concentration.test.ts', '__integration__/standing-and-seats.test.ts'] },
      ...
      { code: 'L2.7.3', name: 'Data and access', domain: 'REGULATORY', processes: [
        ...
        { code: 'L3.7.3.2', name: 'Company walls', owner: 'Etyme', status: B,
          tasks: ['Filtered at the query, not the screen',
            ...
            'Disclosure is a term on the agreement between client and prime, off by default, amended on the version trail like every other term',
        ...
        { code: 'L3.7.3.4', name: 'What we tell people about their data', owner: 'Compliance', status: B,
          tasks: ['Terms, a privacy notice and a DPA as ordinary pages, in the product\'s own look',
            ...
            'Every category carries its retention line, citing the federal rule or stating that none can be cited (lib/retention SCHEDULE, read by the legal document, never typed beside a category)',
            ...
            'The six things a procurement lead asks first are answered on every legal page, each line linking to the section that answers it',
            'The three legal pages share one document component, so they cannot drift apart',
            'No legal page scrolls sideways at phone width'],
        ...
        { code: 'L3.7.3.6', name: 'Retention, export, erasure and the breach clock', owner: 'Compliance', status: P,
          tasks: [
            'DONE: A person asks for everything held about them, or asks to be forgotten, from their own page and without writing to anybody',
            'DONE: The clock is a stored date with the reason for it in words, because the period is counsel\u2019s answer and a deadline nobody can explain is a number nobody can stand behind',
            'DONE: Erasure is anonymization and never a row delete: the invoice still foots, the signed week still has its hours, the days on site are still the client\u2019s exposure, and none of them names anybody',
            'DONE: The tombstone is unique and unroutable \u2014 a reserved .invalid address \u2014 so a person who asked to be forgotten can never be written to',
            'DONE: A statutory minimum beats erasure and the request says so: HELD rather than refused, with what was kept and why in the person\u2019s own words',
            'DONE: The schedule is code, one period per category in the privacy notice\u2019s own list, and never a per-company setting \u2014 and where no federal minimum can be cited the line states none and says why',
            'DONE: A legal hold is a row with a reason, a name and a review date, and two companies can hold one person at once without either releasing the other\u2019s hold',
            'DONE: A breach is its own record and not a kind of error report: discovered at, whether personal data was involved, whose, which customers, and a clock per audience',
            'DONE: The nightly sweep tells staff a day before a clock runs out and goes on saying so after it has, once a day rather than once a run',
            ...
            'OPEN: nothing ages out a person who has simply gone quiet, and RETENTION_ANONYMIZE stays in autonomy\u2019s PLANNED list because nothing writes it \u2014 the tombstone is the anonymization and there is no second occasion for the act'],
          implementedBy: [
      ...
      { code: 'L2.7.5', name: 'Offers', domain: 'MARKET', processes: [
        ...
        { code: 'L3.7.5.1', name: 'Program office as a service (MSP)', owner: 'Etyme', status: P,
          tasks: [
            'Etyme runs a client\u2019s contractor program as a vendor-neutral program office, funded the way an MSP is funded, by a percentage the suppliers pay on their billings.',
            ...
            'OPEN, FOUNDER: The fee model \u2014 supplier-funded, a percentage on billings, disclosed to every supplier when they join rather than discovered later. No number exists: the price is set after real clients are using it, and no agent invents one.',
            ...
            'REWRITTEN TWICE ON 2026-09-20, and the second time is the one that stands. It first said the incumbents\u2019 MSPs stay partners and this offer is only for the clients they will not take. That was replaced in the morning by its opposite \u2014 the incumbents\u2019 program offices are who Etyme is measured against \u2014 and the founder replaced THAT the same day, in these words: \u201cMSP selling should be undercover selling and more as value addition rather than fully pitching for the market. Remove the threat if any to supply chain.\u201d So the rivalry framing is gone and nothing replaces it: the record is what the page sells, the program office appears once as a quiet second way to use it, never as a headline, never sized against anybody, never as a new category. Nothing on any page, letter or screen reads as a threat to a supplier \u2014 its rates and its sub-vendors\u2019 names stay private, its client stays its client, Etyme never supplies a person, and a program Etyme runs is said to make a supplier\u2019s life easier rather than to watch it. A sentence a supplier would read as aimed at them is removed, and lib/positioning holds a guard for it. Both earlier versions are kept here because each was written down confidently and each was wrong within hours; a row that shows only the latest teaches nobody that.',
