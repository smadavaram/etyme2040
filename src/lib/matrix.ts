      ...
      { code: 'L2.1.4', name: 'Reaching the market, and moving work between firms', domain: 'MARKET', processes: [
        ...
        { code: 'L3.1.4.6', name: 'The contractor census', owner: 'Etyme', status: P,
          ...
          tasks: [
            ...
            'Regulatory OPEN \u2014 six of the seven census automation names cannot be written: AutomationLog.companyId is a required foreign key and a census has no company until its rows are imported, and the deletion destroys that sandbox, so a row written there would cascade away at the moment somebody audits it. companyId String? frees all six; until then the record is the CensusRequest row itself',
            'Regulatory OPEN \u2014 a census file opened before the rows are imported writes no AccessLog row, because AccessLog.subjectId is a required foreign key to Person and the contractors are still lines in an unparsed file. The counter on the file moves and staff are told; subjectId String? with the census named instead is the fix',
            'Regulatory OPEN \u2014 CENSUS_CLOCK_WARNED remembers that it warned by the notification it wrote to the named person, so a census whose assigned address has no Person row warns once per run rather than once per day. CensusRequest.lastWarnedAt, the column Breach already has, would close it',
